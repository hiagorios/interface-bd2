-- Antes de inserir um evento, é checado se o id_organizador realmente é de um organizador
CREATE OR REPLACE FUNCTION insert_evento_trigger_func() RETURNS TRIGGER AS
$$ 
BEGIN
	IF NOT EXISTS (SELECT 1 from usuario where tipo_usuario = 'O' AND id = NEW.id_organizador) THEN
